<section id="ulasan" class="py-20 bg-[#F5EFE2]">
    ...
    <div id="reviewModal" x-data="reviewApp()" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div :class="step === 1 ? 'sm:max-w-xl' : 'sm:max-w-4xl'" class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle w-full p-6 sm:p-7 border border-[#C9A227]/20">
                <div>
                    <div class="flex justify-between items-center mb-4 pb-3 border-b border-neutral-100">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 mt-0.5" x-show="step === 1">Verifikasi kode pesanan Anda</p>
                        </div>
                        <button @click="closeModal()" 
                                class="w-8 h-8 rounded-full bg-neutral-100 hover:bg-neutral-200 text-neutral-400 hover:text-neutral-700 flex items-center justify-center transition-colors shrink-0"
                                title="Tutup">
                    ...
                    <div x-show="step === 1">
                        <p class="text-xs sm:text-sm text-gray-600 mb-4 leading-relaxed">
                            Masukkan kode pesanan Anda untuk menulis ulasan produk yang Anda nikmati. Contoh: <code class="bg-neutral-100 px-1.5 py-0.5 rounded text-[#7A1F2B] font-mono text-xs font-bold">RM-X8V3AQ-13092026</code>
                        </p>
                        <div class="space-y-4 text-sm">
                            <div>
                                <label class="block text-gray-700 font-bold mb-2 text-xs uppercase tracking-wider" for="order_number">Kode Pesanan</label>
                                <input x-model="order_number" class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#7A1F2B]/20 focus:border-[#7A1F2B] text-sm" id="order_number" type="text" placeholder="Masukkan Kode Pesanan">
                            </div>
                            <div x-show="error_message" class="text-red-500 text-xs font-semibold" x-text="error_message"></div>
                        </div>
                        <div class="mt-6 flex justify-end gap-3">
                            <button type="button" @click="closeModal()" class="px-4 py-2 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 text-sm transition-colors">
                                Batal
                            </button>
                            <button type="button" @click="checkOrder()" class="px-6 py-2 bg-[#7A1F2B] text-white font-medium rounded-xl hover:bg-[#611922] text-sm transition-colors" :disabled="isLoading">
