...
- **Frontend & Fitur Terputus**:
  ...
  - **Penamaan Kategori Hidangan (*Topping / Toping* -> *Menu Tambahan*)**:
    - Mengubah nama kategori hidangan dari `Topping` / `Toping & Bumbu` / `Lauk Tambahan` menjadi `Menu Tambahan` pada seluruh sistem database (`menu_categories`), seeder (`DatabaseSeeder.php`), filter etalase beranda web (`HomeController.php`), dan panel admin.
